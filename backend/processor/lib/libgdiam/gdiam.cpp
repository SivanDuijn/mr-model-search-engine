...
#define GDIAM_HEAP_LIMIT 10000
#define GDIAM_HEAP_DELTA 10000
#define GDIAM_PI 3.14159265958979323846
...
namespace gdiam
{
                              ...
                              gdiam_point_cnt proj)
    {
        ...
        for (int ind = 0; ind < 8; ind++)
        {
            ...
            for (int jnd = 0; jnd < 8; jnd++)
            {
                ...
                rl = gdiam_max(rl, pnt_distance(a, b, proj));
            }
        }
    ...
    public:
                  ...
                  gdiam_point proj_dir, gdiam_real dist)
        {
            ...
            bb.init(left->getBBox(), right->getBBox());
            max_diam = gdiam_max(gdiam_max(bbox_proj_dist(_left->getBBox(),
                                                          _right->getBBox(),
                                                          proj_dir),
        ...
        double getProjectionError() const
        {
            ...
            two_r = gdiam_max(left->maxDiam(), right->maxDiam());
...
#define UDM_BIG_SAMPLE 2
    ...
    void GTreeDiamAlg::compute_by_heap(double eps)
    {
        ...
        heap_limit = GDIAM_HEAP_LIMIT;
        heap_delta = GDIAM_HEAP_DELTA;
...
#define UMD_SIZE 5
    ...
    void convex_hull(vec_point_2d &in, vec_point_2d &out)
    {
        ...
        out.push_back(in[minmin]);
        ++top; // push  minmin point onto stack
        i = minmax;
        while (++i <= maxmin)
        {
    ...
    public:
       ...
       point and slope */
                              ...
                              gdiam_real &y)
        {
            ...
            if (eq_real(a_angle, 0.0) || eq_real(a_angle, GDIAM_PI))
            {
                x = ch[b_ind]->x;
                y = ch[a_ind]->y;
                return;
            }
            if (eq_real(a_angle, GDIAM_PI / 2.0) || eq_real(a_angle, GDIAM_PI * 1.5))
            {
                x = ch[a_ind]->x;
                y = ch[b_ind]->y;
                return;
                        ...
                        gdiam_real &angle_3)
        {
            ...
            angle = angles[ind];
            angle_1 = angle + GDIAM_PI * 0.5;

            if (angle_1 >= (2.0 * GDIAM_PI))
                angle_1 -= 2.0 * GDIAM_PI;

            angle_2 = angle + GDIAM_PI;
            if (angle_2 >= (2.0 * GDIAM_PI))
                angle_2 -= 2.0 * GDIAM_PI;

            angle_3 = angle + 1.5 * GDIAM_PI;
            if (angle_3 >= (2.0 * GDIAM_PI))
                angle_3 -= 2.0 * GDIAM_PI;
        }
...
#define ZERO_EPS (1e-6)
    ...
    public:
        ...
        void compute()
        {
            ...
            if ((!(fabs(pnt_dot_prod(base_proj, out_1)) < 1e-6)) || (!(fabs(pnt_dot_prod(base_proj, out_2)) < 1e-6)) || (!(fabs(pnt_dot_prod(out_1, out_2)) < 1e-6)))
            {

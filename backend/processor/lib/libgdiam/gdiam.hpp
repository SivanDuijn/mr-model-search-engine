...
#include <algorithm>
#include <assert.h>
#define gdiam_min std::min
#define gdiam_max std::max
...
namespace gdiam
{
                            ...
                            const gdiam_point q)
    {
        ...
        return ((GDIAM_FLOAT_ZERO(p[0] - q[0])) && (GDIAM_FLOAT_ZERO(p[0] - q[0])) && (GDIAM_FLOAT_ZERO(p[0] - q[0])));
    }
    ...
    public:
                  ...
                  const GBBox &b)
        {
            ...
            {
                min_coords[ind] = gdiam_min(a.min_coords[ind],
                                            b.min_coords[ind]);
                max_coords[ind] = gdiam_max(a.max_coords[ind],
                                            b.max_coords[ind]);
            }
        }
        void dump() const
    ...
    public:
                  ...
                  const gdiam_point _dir_3)
        {
            ...
            if ((!(fabs(pnt_dot_prod(dir_1, dir_2)) < 1e-6)) || (!(fabs(pnt_dot_prod(dir_1, dir_3)) < 1e-6)) || (!(fabs(pnt_dot_prod(dir_2, dir_3)) < 1e-6)))
            {
